...
show_help() {
    cat << 'EOF'
sequential_queue.sh v4.0.0 - Universal sequential execution queue
...
ATOMIFICATION:
    ...
    - Each file is processed sequentially with full queue management
    - All tool checks and special handling are preserved
...
ATOMIC vs NON-ATOMIC:
    ...
    BAD (non-atomic) - but will be automatically atomified:
...
CUSTOM_LOG_DIR=""
ATOMIFY=1  # Enable atomification by default
MAIN_TIMEOUT=""
MAIN_PIPELINE_TIMEOUT=""
MAIN_MEMORY_LIMIT=""
MAIN_VERBOSE=""
...
log() {
    ...
    echo "$msg" >> "${GLOBAL_LOG_FILE:-/dev/null}"
}
...
check_pipeline_timeout() {
    local timeout="${1:-${PIPELINE_TIMEOUT:-86400}}"
    
    if [ ! -f "$PIPELINE_TIMEOUT_FILE" ]; then
        # First process in pipeline - set timeout
        echo "$(date +%s):$timeout" > "$PIPELINE_TIMEOUT_FILE"
        log INFO "Pipeline timeout set to ${timeout}s"

        # Start timeout monitor in background
        (
            sleep "$timeout"
            if [ -f "$PIPELINE_TIMEOUT_FILE" ]; then
                log ERROR "PIPELINE TIMEOUT after ${timeout}s - killing all processes"
        ...
        if [ $elapsed -gt "$timeout_val" ]; then
            ...
            echo "$(date +%s):$timeout" > "$PIPELINE_TIMEOUT_FILE"
            log INFO "Pipeline timeout reset to ${timeout}s"
        fi
...
# Execute a single command with full sequential queue logic
execute_single_command() {
    local cmd=("$@")
    local exit_code=0
    
    # Set up per-command environment
    local TIMEOUT="${MAIN_TIMEOUT:-${TIMEOUT:-86400}}"
    local PIPELINE_TIMEOUT="${MAIN_PIPELINE_TIMEOUT:-${PIPELINE_TIMEOUT:-86400}}"
    local MEMORY_LIMIT_MB="${MAIN_MEMORY_LIMIT:-${MEMORY_LIMIT_MB:-2048}}"
    local VERBOSE="${MAIN_VERBOSE:-${VERBOSE:-0}}"
    
    # Create per-command log file
    local EXEC_LOG="${LOGS_DIR}/sequential_queue_$(date '+%Y%m%d_%H%M%S')_$$.log"
    
    # Extract command and args
    local COMMAND="${cmd[0]}"
    local ARGS=("${cmd[@]:1}")
    
    log INFO "Processing command: $COMMAND ${ARGS[*]}"
    ...
    check_pipeline_timeout "$PIPELINE_TIMEOUT"
        ...
        if [ "$HOLDER_PID" -gt 0 ]; then
            ...
            if kill -0 "$HOLDER_PID" 2>/dev/null; then
                ...
                if [ $((WAIT_COUNT % 60)) -eq 0 ] && [ $WAIT_COUNT -gt 0 ]; then
                    local holder_cmd
                    holder_cmd=$(ps -p "$HOLDER_PID" -o args= 2>/dev/null | head -1 || echo "unknown")
                    local wait_time=$((WAIT_COUNT))
                    log INFO "Still waiting for PID $HOLDER_PID: $holder_cmd (${wait_time}s elapsed)"
...
# Set up global log file for atomification phase
GLOBAL_LOG_FILE="${LOGS_DIR}/sequential_queue_$(date '+%Y%m%d_%H%M%S')_$$.log"

# Check if atomification is needed
ATOMIFY_NEEDED=0
ATOMIC_COMMANDS=()

# Source atomifier if available and enabled
ATOMIFIER_SCRIPT="${SCRIPT_DIR}/tool_atomifier.sh"
if [[ $ATOMIFY -eq 1 ]] && [[ -f "$ATOMIFIER_SCRIPT" ]]; then
    source "$ATOMIFIER_SCRIPT"
    
    # Generate atomic commands
    log INFO "Checking if command needs atomification: $MAIN_COMMAND ${MAIN_ARGS[*]}"
    mapfile -t ATOMIC_COMMANDS < <(generate_atomic_commands "$MAIN_COMMAND" "${MAIN_ARGS[@]}" | grep "^ATOMIC:")
    
    if [[ ${#ATOMIC_COMMANDS[@]} -gt 1 ]]; then
        log INFO "Command will be atomified into ${#ATOMIC_COMMANDS[@]} atomic operations"
        ATOMIFY_NEEDED=1
    else
        log INFO "Command does not need atomification"
    fi
fi

# Execute the command(s)
if [[ $ATOMIFY_NEEDED -eq 1 ]]; then
    log INFO "Executing ${#ATOMIC_COMMANDS[@]} atomic commands sequentially"
    
    # Execute each atomic command
    cmd_index=0
    overall_exit_code=0
    
    for atomic_cmd in "${ATOMIC_COMMANDS[@]}"; do
        ((cmd_index++))
        # Remove ATOMIC: prefix and parse command
        actual_cmd="${atomic_cmd#ATOMIC:}"
        # Remove leading/trailing whitespace
        actual_cmd="${actual_cmd#"${actual_cmd%%[![:space:]]*}"}"
        actual_cmd="${actual_cmd%"${actual_cmd##*[![:space:]]}"}"
        
        log INFO "[${cmd_index}/${#ATOMIC_COMMANDS[@]}] Processing: $actual_cmd"
        log DEBUG "Raw atomic command: $atomic_cmd"
        
        # Parse the command into array
        # Skip "wait_all.sh --" prefix if present
        if [[ "$actual_cmd" =~ ^wait_all\.sh[[:space:]]+--[[:space:]]+ ]]; then
            actual_cmd="${actual_cmd#wait_all.sh -- }"
        fi
        
        # Execute the atomic command with full sequential queue logic
        # Parse the command string into an array and call the function
        cmd_array=()
        eval "cmd_array=($actual_cmd)"
        execute_single_command "${cmd_array[@]}"
        exit_code=$?
        
        if [[ $exit_code -ne 0 ]]; then
            log ERROR "Atomic command ${cmd_index} failed with exit code: $exit_code"
    ...
    if [[ $overall_exit_code -eq 0 ]]; then
        log INFO "All ${#ATOMIC_COMMANDS[@]} atomic commands completed successfully"
    fi
    
    exit $overall_exit_code
else
    # Single command execution
    execute_single_command "$MAIN_COMMAND" "${MAIN_ARGS[@]}"
    exit $?
fi