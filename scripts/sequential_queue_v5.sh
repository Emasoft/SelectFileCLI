...
# Version: 5.0.0
#
# This version correctly implements atomification by:
# 1. Detecting atomifiable commands
# 2. Generating atomic commands
# 3. Calling sequential_queue.sh recursively for each atomic command
# 4. Each atomic command goes through the FULL default sequence
#
set -euo pipefail
...
show_help() {
    cat << 'EOF'
sequential_queue.sh v5.0.0 - Universal sequential execution queue
...
OPTIONS:
    ...
    --internal-atomic      Internal flag: this is an atomic command (do not atomify)
...
ATOMIFICATION:
    ...
    - Each atomic command is queued separately with full processing
...
IS_ATOMIC=0  # This is not an atomic command by default
PARSED_TIMEOUT=""
...
# If atomification is enabled and this is not already an atomic command
if [[ $ATOMIFY -eq 1 ]] && [[ $IS_ATOMIC -eq 0 ]]; then
    ...
    if [[ -f "$ATOMIFIER_SCRIPT" ]]; then
        ...
        if [[ ${#ATOMIC_COMMANDS[@]} -gt 1 ]]; then
            ...
            [[ "${PARSED_VERBOSE:-0}" -eq 1 ]] && echo "[SEQ-QUEUE] DEBUG: Found ${#ATOMIC_COMMANDS[@]} atomic commands" >&2
            
            # Build option string to pass to atomic commands
            OPTS=""
            [[ -n "$PARSED_TIMEOUT" ]] && OPTS="$OPTS --timeout $PARSED_TIMEOUT"
            [[ -n "$PARSED_PIPELINE_TIMEOUT" ]] && OPTS="$OPTS --pipeline-timeout $PARSED_PIPELINE_TIMEOUT"
            [[ -n "$PARSED_MEMORY_LIMIT" ]] && OPTS="$OPTS --memory-limit $PARSED_MEMORY_LIMIT"
            [[ -n "$CUSTOM_LOG_DIR" ]] && OPTS="$OPTS --log-dir \"$CUSTOM_LOG_DIR\""
            [[ -n "$PARSED_VERBOSE" ]] && OPTS="$OPTS --verbose"
            
            # Execute each atomic command by calling sequential_queue.sh recursively
            overall_exit_code=0
            cmd_index=0
            
            [[ "${VERBOSE:-0}" -eq 1 ]] && echo "[SEQ-QUEUE] DEBUG: Starting atomic command loop" >&2
            for atomic_cmd in "${ATOMIC_COMMANDS[@]}"; do
                [[ "${VERBOSE:-0}" -eq 1 ]] && echo "[SEQ-QUEUE] DEBUG: Processing atomic_cmd='$atomic_cmd'" >&2
                ((cmd_index++)) || true
                ...
                echo "[SEQ-QUEUE] [${cmd_index}/${#ATOMIC_COMMANDS[@]}] Queueing atomic command: $actual_cmd" >&2
                
                # Call sequential_queue.sh recursively with --internal-atomic flag
                # Build command string
                recursive_cmd="$0"
                [[ -n "$PARSED_TIMEOUT" ]] && recursive_cmd="$recursive_cmd --timeout $PARSED_TIMEOUT"
                [[ -n "$PARSED_PIPELINE_TIMEOUT" ]] && recursive_cmd="$recursive_cmd --pipeline-timeout $PARSED_PIPELINE_TIMEOUT"
                [[ -n "$PARSED_MEMORY_LIMIT" ]] && recursive_cmd="$recursive_cmd --memory-limit $PARSED_MEMORY_LIMIT"
                [[ -n "$CUSTOM_LOG_DIR" ]] && recursive_cmd="$recursive_cmd --log-dir \"$CUSTOM_LOG_DIR\""
                [[ -n "$PARSED_VERBOSE" ]] && recursive_cmd="$recursive_cmd --verbose"
                recursive_cmd="$recursive_cmd --internal-atomic -- $actual_cmd"
                
                [[ "${VERBOSE:-0}" -eq 1 ]] && echo "[SEQ-QUEUE] DEBUG: Executing: $recursive_cmd" >&2
                $recursive_cmd
                exit_code=$?
                
                if [[ $exit_code -ne 0 ]]; then
                    echo "[SEQ-QUEUE] Atomic command ${cmd_index} failed with exit code: $exit_code" >&2
...
# 2. This is already an atomic command
# 3. The command doesn't need atomification
# 4. Atomifier is not available
# So proceed with the DEFAULT SEQUENCE
